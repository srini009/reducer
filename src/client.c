...
reducer_return_t reducer_metric_reduce(const char *ns, const char *name, const char* key_start, int32_t agg_id, reducer_metric_reduction_op_t op, reducer_metric_handle_t handle)
{
    if(!ns || !name)
